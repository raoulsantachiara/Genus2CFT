...
The Eq. ~\eqref{conformal_block_decomposition} provides a small $z$ expansion of the modular conformal block of the type:
\begin{equation}
\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z) =z^{|\boldsymbol{h}|-2h_{\sigma_N}} \;\sum_{j=1}^{\infty} a_j \; z^j.
\end{equation}
The computation of the $a_j$ coefficients becomes quickly impossible to accomplish and one has to approximate $\mathcal{G}_{c,\boldsymbol{h}}^{(3)}(z)$ by truncating the above sum above some value $L$:
\begin{equation}
\label{trunc}
\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z) \sim z^{|\boldsymbol{h}|-2h_{\sigma_N}} \;\sum_{j=1}^{L} a_j \; z^j.
\end{equation}
For $N=3$, for instance, we are able to reach $L=6$.
...
\noindent Here we show how to improve the expansion in \cite{Collier} by using a similar but different procedure.  First we use the truncated sum \eqref{trunc} to express the $\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z)$ as  
...
\end{equation}
where we have  denoted by $\mathcal{F}_{N c, |h }(z)$ the Virasoro sphere conformal blocks with the four external dimensions fixed to  $h_{\sigma_N}$ and with internal channel of dimension $h$.  From Eq. \eqref{trunc}, the coefficients $\alpha_{l}^{\boldsymbol{h}}$ can be easily derived. The Eq.\eqref{expansion_sphere_conf_blocks} is already a better approximations than with Eq. ~\eqref{expansion_sphere_conf_blocks}, as each  $\mathcal{F}_{Nc, |\boldsymbol{h}|+l}(z)$ can be computed till orders much bigger than $z^{|\boldsymbol{h}|-2h_{\sigma_N}+L}$. In other words,  this means that we can take into account the contributions of certain very low descendants of the orbifold algebra that were previously unaccessible.

\noindent The expansion \eqref{expansion_sphere_conf_blocks} is natural as the orbifold algebra admits as a sub-algebra a Virasoro algebra of charge $N c$\cite{Borisov}. Note that the fine structure of the orbifold algebra was used in \cite{Dupic} to derive differential equation for certain twist correlation function. A more detail discussion concerning the meaning of  \eqref{expansion_sphere_conf_blocks}  as well as the orbifold algebra nature of the $\alpha_{l}^{\boldsymbol{h}}$ are discussed below in \ref{deralpha}.
