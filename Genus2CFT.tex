...
$c$, living on a Riemann surface $\Sigma_g$ of genus $g$.
...
\mathcal{Z}_{N-1}(x)=\text{Partition function of $\mathcal{C}$ on the surface  $\Sigma_{N-1}(x)$, defined in \ref{riemann_surf}}.
 \end{equation}
The CFT partition function on $\Sigma_{N-1}(x)$ is strictly related to the orbiforld CFT $\mathcal{C}^{\otimes N}/Z_N$ living on the plane $z$ \cite{Dixon, Knizhnik}. In this respect a crucial role is played by the $\mathbb Z_N$ twist and anti-twist field $\sigma_N$ and $\bar{\sigma}_N$, that are  spinless primary field of conformal dimension~\cite{Knizhnik}
\begin{equation}
...
\noindent Following~\cite{Lunin}, the partition function $\mathcal{Z}_{N-1}$ can be put in relation to the $\mathcal{C}$ partition function $\mathcal{\hat{Z}}_{N-1}$ on a Riemann surfaces with a flat metric:  
\begin{equation}\label{partition_twist_correl}
 \mathcal{\hat{Z}}_{N-1}(x)=e^{cS_{\text{anom.}}(x)} \mathcal{Z}_{N-1}(x),
\end{equation}
where the prefactor $e^{cS_{\text{anom.}}(x)}$ in Eq.~\eqref{partition_twist_correl} is the Weyl anomaly originating from the Weyl transformation between the different metrics.
...
\noindent The Weyl anomaly can be explictely calculated. For instance, for $N=2$, the CFT partition function on a flat torus 
with modulus $\tau$, see Eq.~\eqref{tau}, is~\cite{Lunin}
\begin{equation}\label{partition_torus_twist}
 \mathcal{\hat{Z}}_1(x)=|2^8 x(1-x)|^{c/12}\langle \sigma_2|\bar{\sigma}_2(1)\sigma_2(x, \bar{x})|\bar{\sigma}_2\rangle.
\end{equation}
