...
The computation of the small $z$ epxansion of $\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z)$ becomes effectively impossible to accomplish. For instance, for $N=3$ we could compute the expansion coefficients till to the order  $z^6$.  The global properties of the twist correlation functions, and in particular its crossing symmetry \eqref{cross_symmetry}, are missed by using in Eq.~\eqref{conformal_block_decomposition_0} the small $z$ expansion of the modular conformal block. The convergence of the series is slow near $|z|=1$.

In \cite{Collier}, this problem was tackled by using the pillow frame. Here we show an slightly better procedure that is based on two steps: i) expanding the  $\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z)$ in terms of sphere conformal blocks of charge $N c$, ii) using the elliptic Zamolodchikov recursion relation. 
 
In order to drastically impr ove the converge of the numerical bootstrap, it is necessary to expand the higher-genus conformal blocks $\mathcal{G}_{c,\boldsymbol{h}}^{(N)}(z)$ as a linear combination of sphere conformal blocks with central charge $Nc$. More specifically, we shall expect
