...
unknowns $D_{\boldsymbol{h},\boldsymbol{\bar{h}}}$.
...
at level $L$, it is possible to obtain a set of structure constants $D_{\boldsymbol{h},\boldsymbol{\bar{h}}}(L)$ for any random realization of the points $\{z_j\}$. If the bootstrap converges,  we expect that the variance of the solutions $D_{\boldsymbol{h},\boldsymbol{\bar{h}}}(L)$ will be small and that $D_{\boldsymbol{h},\boldsymbol{\bar{h}}}(L)\to D_{\boldsymbol{h},\boldsymbol{\bar{h}}}$, for $L\to \infty$. In order to drastically improve the convergence of the numerical bootstrap, it is necessary 
to approximate the orbifold conformal block $\mathcal{G}_{c, \boldsymbol{h}}^{(N)}(z)$ as 
in Eq.~\eqref{expansion_sphere_conf_blocks_trunc}, in terms of sphere conformal blocks, and apply the elliptic recursion relation
of Eq.~\eqref{zam_recursion}, truncated at some level $L'$, to calculate the latter. 
...
\end{equation}
for $c=7/10$ and the sum running over the conformal dimensions in Eq.~\eqref{tric_ising_fields}.
...
for each sample of random points. The approximation of Eq.~\eqref{genus_one_conf_block} for the 
regularized orbifold conformal blocks has been truncated at level $L=6$. The 
elliptic recursion for the sphere conformal blocks has also been truncated at level $L'=6$.
...
coefficient of variation. In the third column, we write the expect value for each structure constant.}
\end{table}



We have determined the genus two partition function ($N=3$) by solving the numerical bootstrap 
for the following  minimal models: The Ising field theory ($c=1/2$), the  Lee-Yang model ($c=-22/5$), 
and the \textit{Gaffnian} theory ($c=-3/5$) \cite{Simon,Ardonne}. Note that the last two CFTs are non-unitary. All of them 
belong to the $A$-series of minimal models and, therefore, we expect that the conformal block expansion of Eq.~\eqref{conformal_block_decomposition_0} is still diagonal, i.e. $D_{\boldsymbol{h}, \boldsymbol{\bar{h}}}\propto \delta_{\boldsymbol{h}, \boldsymbol{\bar{h}}}$ (here $\boldsymbol{h}=\{h_1, h_2, h_3\}$). Moreover, as we obtained in Eq.~\eqref{D_3}, 
in the genus two case the structure constants in Eq.~\eqref{conformal_block_decomposition_0} are proportional to the three-point coefficients of the seed theory on the sphere, 
$D_{\boldsymbol{h}, \boldsymbol{\bar{h}}}\propto (C_{\boldsymbol{h},\boldsymbol{\bar{h}}}^{\text{seed}})^2$. In Appendix~\ref{app_min_mod}, we remind the 
...
the mean and the coefficient of variance obtained for the square of the structure constants 
$(C_{\boldsymbol{h}, \boldsymbol{\bar{h}}}^{\text{seed}})^2$ of the seed theory after solving 
Eq.~\eqref{cross_symmetry_2} with each set of random points. The third column collects 
the exact value of these structure constants calculated from xxx. The agreement between the 
...
\hline 
  \multirow{2}{*}{$(h_1, h_2, h_3)$} &  \multicolumn{2}{c|}{$(C_{\boldsymbol{h}, \boldsymbol{h}}^{\text{seed}})^2$} \\
  \cline{2-3}
