...
$c$. We are interested in calculating its partition function on on a compact Riemann surface of genus $g$.
In particular, we consider the family of Renyi Riemann surfaces $\Sigma_g(x)$ of 
...
\end{equation}
which has branch points of order $N$ at $z_{\text{branch}}=0$, $x$, $1$, and $\infty$. The Eq.~\eqref{riemann_surf} can be interpreted as an $N$-sheet cover of the Riemann sphere $\mathbb{S}= \mathbb C\cup \{\infty\}$ parametrized by $z$. For a visualization of the surface on $\mathbb C^2$ we refer to~\cite{Dubrovin}. These Riemann surfaces are parametrized by  
...
This transformation amounts to a cyclic permutation of the $N$ sheets of the surface, where each sheet is labelled by the choice of the branch of the $N$-root in Eq.~\eqref{riemann_surf}. The case $N=2$ is conformally equivalent, under the Abel-Jacobi map, to a flat torus of modulus 
...
\mathcal{Z}_{N-1}(x)=\text{Partition function of $\mathcal{C}$ on the surface  $\Sigma_{N-1}(x)$, defined in \ref{riemann_surf}}.
 \end{equation}
The case of our interest will be in particular the case $N=3$, that is a compact surface with two holes. 
 
A very useful observation is that the CFT partition function on $\Sigma_g(x)$ is strictly related to the orbiforld CFT $\mathcal{C}^{\otimes N}/Z_N$ living on the sphere  $\mathbb S$. This relation can be understood from the fact that the fields in the $k$-th replica correspond to the ones living on the $k$-th sheet of $\Sigma_g$ while the orbifold takes into account to previously mentioned $Z_N$ symmetry of $\Sigma_g(x)$\cite{Dixon, Knizhnik}. 
In the orbifold theory a crucial role is played by the $\mathbb Z_N$ twist fields $\sigma_N$, that are  spinless primary field of conformal dimension~\cite{Knizhnik}
\begin{equation}
...
\noindent Following~\cite{Lunin}, the partition function $Z_g$ can be calculated as the four-point function of twist field~\cite{Lunin}
\begin{equation}\label{partition_twist_correl}
 \mathcal{Z}_{N-1}(x)=e^{cS_{\text{anom.}}(x)}\langle \sigma_N| \bar{\sigma}_N(1)\sigma_N(x,\bar{x})|\bar{\sigma}_N\rangle.
\end{equation}
...
The rest of the paper will be devoted to expansion of the twist field four-point 
function in terms of modular conformal blocks and we will focus in particular on 
the symmetry $x\mapsto 1-x$. The idea of using the crossing simmetry of the twist field correlation function to calculate higher genus CFT partition function was firstly  proposed in~\cite{ZamolodchikovAT}.
