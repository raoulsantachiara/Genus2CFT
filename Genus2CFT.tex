...
The function $\mathcal{G}_{c, \boldsymbol{h}}^{(N)}(z)$, discussed below, are the so-called modular conformal blocks. The $\mathcal{G}_{c, \boldsymbol{h}}^{(N)}(z)$, normalized such that: 
\begin{equation}
\mathcal{G}_{c, \boldsymbol{h}}^{(N)}(z)=z^{\;h -2\; h_{\sigma_N}}\left(1+ O(z)\right),
\end{equation}
are the so called modular conformal blocks and are fixed by the orbifold symmetry algebra. These functions encode therefore the universal part of the orbifold correlation functions. On the other hand, the structure constants  $D_{\boldsymbol{h}, \boldsymbol{\bar{h}}}$  characterize the specific bootstrap solution under consideration. Finally note that in Eq.\eqref{conformal_block_decomposition_0}, we inserted one twist field at a general point $(z,\bar{z})$ on the Riemann sphere to stress the fact we are computing a standard CFT correlation function on the $z$-sphere. To recover the geometric interpretations encoded in Eq.~\eqref{riemann_surf} and in Eq.~\eqref{geom_inter}, one sets $z,\bar{z}\to x,\bar{x}$.
...
\end{equation}
one usually considers a $N$-to-one conformal map
$t\mapsto z(t)$ with branch points at $z_{\text{branch}}=\{0, 1\}$ such that, near 
these points, it behaves as $z-z_{\text{branch}}\sim (t-t_{\text{branch}})^N$~\cite{Lunin}.
That is, the $t$-surface is a $N$-sheet cover with genus zero (a Riemann sphere)
 of the $z$-Riemann sphere with a branch cut along 
$(0,1)$. The point $z=\infty$, where the field $\boldsymbol{X}_{\boldsymbol{h}}^{\boldsymbol{M}}$ in Eq.~\eqref{orbifold_three-point} is inserted, is mapped to $N$ different points $t_\infty=\{t_1, \dots, t_N\}$ in 
the covering space. Let us denote by $\mathcal{X}_{h_j}^{M_j}(t_j)$ one of the images
of the operator $X_{h_j}^{M_j}(\infty)$ under the covering map $t\mapsto z(t)$:
\begin{equation}
\{\mathcal{X}_{h_j}^{M_j}(t_j)\}_{j=1,\cdots,N} \{\boldsymbol{\mathcal{L}}_{-M_j}(t_j)\phi_{h}(t_j)\}_{j=1,\cdots,N}\mapsto \boldsymbol{X}_{\boldsymbol{h}}^{\boldsymbol{M}}(\infty),
\end{equation}
where the $\boldsymbol{\mathcal{L}}_{-M_j}(t_j)$ are the transformed Virasoro generators living on the $t$- surface, see Appendix..
...
\end{equation}
{\bf Raoul: The Weyl anomaly is trivial or not tken into account?}

 

\subsection{Explicit calculations of modular conformal blocks: the case $N=2$}
In the case $N=2$, we can use the unfolding  map
\begin{equation}\label{g_one_covering_map}
 z(t)=\frac{(t+1)^2}{4t},
\end{equation}
according to which $t_{\infty}=\{0,\infty\}\mapsto z=\infty$.
Then Eq.~\eqref{N-point} then becomes:
\begin{equation}\label{C_2_X}
 C_2^{\boldsymbol{h},\boldsymbol{M}}=\langle \mathcal{X}_{h_1}^{M_1} | \mathcal{X}_{h_2}^{M_2}\rangle=
...
\end{equation}
Once the $\boldsymbol{\mathcal{L}}_{-M}(\infty)$and  $\boldsymbol{\mathcal{L}}_{-M}(0)$ are expressed in terms of the $L_{n}$ generators on the Riemann sphere, see Eqs.~\eqref{g1_1} and~\eqref{g1_2}, the matrix $\rho^h$  can be easily computed from Virasoro algebra. 
...
The torus partiction $\mathcal{\hat{Z}}_1(x)$ is a sesquilinear form of irreducible Virasoro characters $\chi_{h}(\tau(x))$~\cite{Cappelli, Cappelli2}. By comparing this expression  to Eqs.~\eqref{partition_torus_twist}  and (\ref{conformal_block_decomposition_0}), one indeed identifies 
\begin{equation}\label{character_conf_block}
\mathcal{G}_{c,h}^{(2)}(x)=2^{-c/3}[x(1-x)]^{-c/24}\chi_{h}(\tau(x)), 
\end{equation}

{\bf Raoul: to fix the good normalization and then the structire constants}

Using Eq.\eqref{C_2_X} in  Eq.~\eqref{conformal_block_decomposition}, one obtains the following expression:
...
matrix products, represented in the diagram of Fig.~\ref{fig-g1}. 
