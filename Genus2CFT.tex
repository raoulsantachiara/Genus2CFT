...
The twist field four-point function of the $\mathcal{C}^{\times N}/\mathbb{Z}_N$ orbifold 
...
a primary field of the seed theory $\mathcal{C}$. The function $\mathcal{G}_{c, \boldsymbol{h}}^{(N)}(z)$, defined below, are the so-called modular conformal blocks. They are normalized such that
\begin{equation}
\label{G_asy}
...
The case of the identity is a little bit different as it contains a null-vector at level one for any value of the central charge. So, 
